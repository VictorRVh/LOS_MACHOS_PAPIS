  ...
  <div class="relative space-y-3">
    ...
    <section class="border border-slate-200 bg-slate-50 px-3 py-3 dark:border-slate-700 dark:bg-slate-800/60">
      <div class="flex items-center gap-2">
        <UserCircleIcon class="h-5 w-5 text-cetpro dark:text-cetpro-light" />
        <div>
      ...
      <div class="mt-3 grid grid-cols-1 gap-3 md:grid-cols-[1.2fr_1fr_1fr_1fr]">
        <div class="flex gap-3 md:col-span-2">
          <FormLabelError label="Tipo de documento *" required class="flex-1" :error="errors.tipo_documento">
            <BaseSelect v-model="formData.tipo_documento" :options="['DNI', 'CARNET EXT.']" />
          </FormLabelError>

          <div class="flex flex-[1.3] items-end gap-2">
            <FormInput v-model="formData.nro_documento" label="Nro. de documento *" :error="errors.nro_documento" />
            <button
              v-if="!edit"
              type="button"
              @click="buscarDNI"
              :disabled="saving"
              class="flex h-[42px] w-[42px] items-center justify-center border border-cetpro bg-cetpro text-white transition hover:bg-cetpro-dark disabled:cursor-not-allowed disabled:opacity-70"
            >
