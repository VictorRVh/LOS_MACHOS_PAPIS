...
const props = defineProps({
  programacionToEdit: { type: Object, default: null },
  periodos: { type: Array, required: true },
  selectedPeriodoId: { type: [String, Number], required: true },
});
...
const schema = yup.object().shape({
  ...
  fecha_inicio: yup.date().required("La fecha y hora de inicio son requeridas."),
  fecha_fin: yup
    .date()
    .required("La fecha y hora de fin son requeridas.")
  ...
  (newVal) => {
    if (newVal) {
      formData.value = {
        ...
        // 🔹 Convertir ISO a formato compatible con datetime-local
        fecha_inicio: newVal.fecha_inicio?.slice(0, 16),
        fecha_fin: newVal.fecha_fin?.slice(0, 16),
      };
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (response) {
      showToast(
        `Programación ${isEditing.value ? "actualizada" : "creada"} con éxito.`,
        "success"
      );
      emit("form-submitted");
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 h-fit sticky top-6">
      ...
      <form @submit.prevent="onSubmit" class="space-y-4">
        ...
        <FormInput
          ...
          label="Título o Tipo de Entrega *"
          :error-message="formErrors.tipo_entrega"
          placeholder="Ej: Sílabo mensual"
        />
        ...
        <div class="flex items-center space-x-3 pt-2">
          ...
          <div>
            <label class="font-medium text-gray-800 dark:text-gray-200">
              Publicar para docentes
            </label>
            <p class="text-xs text-gray-500 dark:text-gray-400">
              Al desmarcar, quedará como borrador.
            </p>
          </div>
        </div>
