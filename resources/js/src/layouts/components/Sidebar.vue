...
const navLinks = [
    ...
    { name: 'Doc. Curricular', routeName: 'documentos', icon: DocumentDuplicateIcon, permissions: ["todo-documento-programado", "icono-documento-programado"] }, // Usaremos un permiso específico
    ...
    <div>
        ...
        >
            <div class="h-20 flex items-center justify-between border-b border-cetpro-white/50 shrink-0 px-4 gap-2">
                ...
                <button @click="layoutStore.toggleSidebar" class="pl-2 rounded-full text-cetpro-text/60 hover:bg-cetpro-light hover:text-white" :class="layoutStore.isSidebarCollapsed ? 'mx-auto' : 'ml-auto'">
                ...
                <ul class="py-0">
                    <li v-for="link in navLinks" :key="link.name">
                        ...
                        <RouterLink v-if="link.routeName === 'start' || hasPermission(link.permissions)" :to="{ name: link.routeName }" v-slot="{ isActive }" class="relative flex flex-col items-center justify-center w-full h-[60px] transition-colors text-cetpro-text/80 hover:bg-cetpro-light hover:text-white group" :class="{ '!bg-cetpro-dark !text-white': isActive }">
                            ...
                            <div v-if="layoutStore.isSidebarCollapsed" class="absolute left-full ml-2 hidden group-hover:block w-max bg-gray-800 text-white text-xs rounded py-1 px-2 z-40">{{ link.name }}</div>
                        </RouterLink>
                    </li>
                </ul>
            </nav>
        ...
        <aside class="fixed inset-y-0 left-0 z-40 w-64 bg-cetpro dark:bg-gray-800 text-cetpro-text flex-col shrink-0 transition-transform duration-300 ease-in-out lg:hidden" :class="layoutStore.isSidebarOpenMobile ? 'translate-x-0' : '-translate-x-full'">
            <div class="h-20 flex items-center justify-between border-b border-cetpro-dark/50 px-4">
                ...
                <button @click="layoutStore.toggleSidebarMobile" class="p-2 text-cetpro-text/80 rounded-full hover:bg-cetpro-light hover:text-white">
...
.custom-scrollbar::-webkit-scrollbar-thumb { background-color: theme('colors.cetpro.dark'); border-radius: 20px; border: 3px solid theme('colors.cetpro.DEFAULT'); }
.custom-scrollbar::-webkit-scrollbar-thumb:hover { background-color: theme('colors.cetpro.light'); }
</style>