...
const setModalData = (data) => {
    ...
    const { title, message, actionButton, returnButton } = data;

    const mergedData = {
        ...initialModalData.value,
        title: title ? title : initialModalData.value.title,
        message: message ? message : initialModalData.value.message,
        actionButton: actionButton ? actionButton : initialModalData.value.actionButton,
        returnButton: returnButton ? returnButton : initialModalData.value.returnButton,
    };
...
<template>
    <Teleport to="body">
        <Transition enter-from-class="opacity-0" leave-to-class="opacity-0"
            ...
            <div v-if="show" id="confirm-modal-overlay" class="fixed inset-0 z-[313] bg-black/60 backdrop-blur-sm">
            </div>
        </Transition>

        <Transition enter-from-class="opacity-0 scale-95" leave-to-class="opacity-0 scale-95"
            enter-active-class="transition transform duration-300 ease-out"
            leave-active-class="transition transform duration-200 ease-in">
            <div v-if="show" class="fixed inset-0 z-[314] flex items-center justify-center p-4" role="dialog"
                aria-modal="true">
                <div
                    class="w-full max-w-md rounded-lg bg-white dark:bg-gray-800 shadow-2xl border dark:border-gray-700">
                    <div class="p-6">
                        <div class="flex items-start gap-4">
                            <div
                                class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/40 sm:mx-0 sm:h-10 sm:w-10">
