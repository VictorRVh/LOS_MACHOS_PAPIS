...
const requiredPermissions = computed(() => {
  return props.cetpro?.id
    ...
    : ["todo-acceso-cetpro", "crear-cetpro"];
});
...
<template>
  <AuthorizationFallback :permissions="requiredPermissions">
    <div class="mt-2 space-y-2 font-inter">
      <h3 class="text-lg font-semibold text-cetpro mb-2">
        {{ isEditing ? "Editar CETPRO" : "Registrar CETPRO" }}
      </h3>

      <hr class="border-t-2 border-cetpro mb-4" />

      <FormInput v-model="formData.cetpro" label="CETPRO" :error="formErrors.cetpro" required />
      <FormInput v-model="formData.rd_autorizacion" label="R.D. de Autorización" :error="formErrors.rd_autorizacion" required />
      ...
      <FormInput v-model="formData.tipo_gestion" label="Tipo de Gestión" :error="formErrors.tipo_gestion" required />
      ...
      <div class="flex gap-2 mt-4">
        <Button
          :title="isEditing ? 'Guardar cambios' : 'Crear CETPRO'"
