...
// 🔥 Total de ingresos por especialidad
...
// ---------------------------------------------
// 🔥 FILTRAR → usando parsearIngresos()
// ---------------------------------------------
        ...
        <div class="p-4 md:p-6 space-y-2"> <!-- Header -->
            ...
            <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                <div class="grid grid-cols-1 md:grid-cols-7 gap-4 items-end"> <!-- Ciclo -->
                    ...
                    <div class="md:col-span-2"> <label
                        ...
                        <BaseSelectGrupo v-model="selectedPeriodo" :options="grupoStore.periodoAnio"
                            label="nombre_periodo" placeholder="Seleccione un periodo" :disabled="!selectedAnio"
            ...
            <div
                ...
                <div v-if="isLoading" class="p-4 space-y-2">
                    <div v-for="i in 5" :key="i" class="h-12 bg-gray-200 dark:bg-gray-700 rounded-md animate-pulse">
                    </div>
                ...
                <Table v-else-if="gruposPaginados.length > 0" :paginacion="true" :current-page="pagina"
                    ...
                    <THead>
                        ...
                        <Th class="border-b border-gray-300 dark:border-gray-600 w-[80px] text-center">N° Estudiantes
                        </Th>
                        <Th class="border-b border-gray-300 dark:border-gray-600 w-[180px] text-center">Docente</Th>
                        <Th class="border-b border-gray-300 dark:border-gray-600 w-[60px] text-center">ingresos</Th>
                    </THead>
                    <TBody> <template v-for="([nombreEspecialidad, modulos]) in gruposAgrupados"
                            ...
                            <tr @click="toggleEspecialidad(nombreEspecialidad)"
                                ...
                                <td colspan="8"
                                    ...
                                    <div class="flex items-center justify-between text-cetpro-text w-full">

                                        <!-- Nombre de especialidad -->
                                        <span>{{ nombreEspecialidad }}</span>

                                        <!-- TOTAL e ícono juntos -->
                                        <div class="flex items-center gap-3">
                                            ...
                                            <!-- Icono que rota -->
                                            <ChevronDownIcon class="h-6 w-6 transition-transform duration-300"
                                                :class="{ 'rotate-180': openEspecialidades.has(nombreEspecialidad) }" />
                                        </div>
                                    </div>
                            ...
                            <tr v-for="(grupo, index) in modulos" :key="grupo.id"
                                v-show="openEspecialidades.has(nombreEspecialidad)"
                                ...
                                <td class="text-center text-green-700 font-semibold py-3">
                                    {{ grupo.cantidad_estudiantes }}
                                </td>

                                <td class="py-3 text-center">  {{ grupo.docente || '---' }}</td>

                                <!-- 🔥 INGRESO POR GRUPO -->
                                <td class="text-center font-bold text-blue-600 py-3">
                                    S/. {{ grupo.ingreso_grupo }}
                                </td>
                            </tr>
                ...
                <div v-else class="text-center py-12"> <svg class="mx-auto h-12 w-12 text-gray-400" fill="none"
                    ...
                    <h3 class="mt-2 text-lg font-semibold text-gray-800 dark:text-gray-200">No se encontraron grupos
                    </h3>
