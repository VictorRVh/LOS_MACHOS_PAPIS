...
const { store: saveSesion, update: updateSesion, destroy: deleteSesion, index: getSesiones } = useHttpRequest(`/api/grupos/${props.id}/sesiones`);
...
const buildAllEvents = () => {
  const holidayEvents = holidays.value.map(h => ({ title: h.title, start: h.date, allDay: true, color: '#6b7280', classNames: ['gcal-event'], display: 'block', id: `gcal-${h.date}` }));
  const sessionBlockEvents = bloquesDeSesiones.value.flatMap(bloque => {
    if (!bloque.dates || !Array.isArray(bloque.dates)) return [];
    return bloque.dates.map(dateStr => ({ id: `${bloque.id}::${dateStr}`, extendedProps: { blockId: bloque.id }, title: bloque.title, start: dateStr, allDay: true, backgroundColor: bloque.color, borderColor: bloque.color }));
  });
  const selectionTemp = selectionEvents.value.map(e => ({ ...e, id: `temp-${e.start}`, title: '' }));
...
const handleDateClick = (clickInfo) => {
  ...
  if (existingIndex > -1) {
    ...
    selectionEvents.value.push({ start: clickedDate, display: 'background', color: 'rgba(51, 139, 191, 0.4)' });
  }
};
...
const handleSaveSesion = async (formData) => {
  ...
  const validDates = allSelectedDates.filter(date => new Date(date + 'T00:00:00').getDay() !== 0 && new Date(date + 'T00:00:00').getDay() !== 6 && !isHoliday(date)).sort();
  if (validDates.length === 0) { closeSlider(); clearSelection(); return; }
  const payload = {
  ...
  if (formData.archivo_sesion) {
    ...
    Object.keys(payload).forEach(key => {
        if(key === 'dates'){ payload[key].forEach(date => fd.append('dates[]', date)); } 
  ...
  try {
    if (editingBlockId.value) {
      await updateSesion(editingBlockId.value, dataToSend);
      ...
      colorIndex++;
      bloquesDeSesiones.value.push({ id: newSesionFromDB.id, title: `Sesión: ${newSesionFromDB.nombre_sesion || 'Sin Tema'}`, description: newSesionFromDB.descripcion, color: newColor, dates: validDates, status: newSesionFromDB.status });
    }
  } catch (error) {
...
const handleTomarAsistenciaHoy = () => {
    const hoy = new Date().toISOString().slice(0, 10);
    const sesionDeHoy = bloquesDeSesiones.value.find(bloque => bloque.dates.includes(hoy));
    if (sesionDeHoy) {
        router.push({ name: 'tomar-asistencia', params: { id: sesionDeHoy.id } });
    } else {
        const fakeId = 'sesion-test-999';
        router.push({ name: 'tomar-asistencia', params: { id: fakeId } });
    }
};
...
onMounted(async () => {
  try {
    const [holidaysData, sesionesData] = await Promise.all([fetchHolidays(), getSesiones()]);
    if (holidaysData && Array.isArray(holidaysData.items)) {
      holidays.value = holidaysData.items.map(item => ({ date: item.start.date, title: item.summary }));
    }
    if (sesionesData && Array.isArray(sesionesData)) {
      bloquesDeSesiones.value = sesionesData.map(sesion => ({ id: sesion.id, title: `Sesión: ${sesion.nombre_sesion || 'Sin Tema'}`, description: sesion.descripcion, color: eventColors[colorIndex++ % eventColors.length], dates: sesion.dates || [], status: sesion.status }));
    }
    buildAllEvents();
  } catch (error) {
        ...
        <div class="p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 h-fit">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-xl font-bold text-gray-800 dark:text-gray-200">Bloques de Sesiones</h2>
                    <button @click="handleTomarAsistenciaHoy" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition-colors text-sm flex items-center gap-2">
                    ...
                    <TBody>
                        ...
                        <Tr v-for="bloque in bloquesDeSesiones" :key="bloque.id">
                            ...
                            <Td class="flex items-center gap-1 text-sm">
                                <button @click="startEditing(bloque.id)" title="Editar Sesión" class="p-1 text-blue-600 hover:text-blue-800 transition-colors">
                                    <PencilIcon class="h-5 w-5" />
                                </button>
                                <button @click="deleteBlock(bloque.id)" title="Eliminar Sesión" class="p-1 text-red-600 hover:text-red-800 transition-colors">
...
.gcal-event { background-color: #6b7280 !important; border-color: #4b5563 !important; cursor: not-allowed; color: white !important; font-weight: bold; }
